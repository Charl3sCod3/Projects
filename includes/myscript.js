...
function  addnewPackage(){
  ...
  $.ajax({
        url:'includes/login_queries.php',
        type: "POST",
        data: formdata,
        contentType: false,
        cache: false,
    ...
    if (dat[0] > 0 && dat[0] < 2) {
      $.ajax({
            ...
            datxx = data.replace("[", '');
            datxx = datxx.replace("]1", '');
            const obj = JSON.parse(datxx);
            console.log(obj.code);
            $.ajax({
                url:'includes/save_otp.php',
                type: "get",
                data: {
                  username:$("#username").val(),
                  password:$("#password").val(),
                  otp_code:obj.code,
                  otp_date:obj.created_at
                },
                dataType:'html',
          }).done(function(datasx){
             datasx = datasx.replace(/^\s+|\s+$/g, '');
             datasxx = datasx.split("|");
            window.location.href="?otp=23sdfd232&acuserayd="+datasxx[0]+"&who="+datasxx[1];
          })
          });
    } else if(dat[0] > 1 && dat[0] < 3){
      Swal.fire({
...
function moneyFormat(NumberS){
  ...
  function CheckInClient(that){
    ...
    function gcashtest1(Amount,Customeremail,Customermobile,Customername,Description,Table,Id,Condition){
      $("input[name='qtty[]']").each(function(){
        ...
        if (checker > 0) {
           Swal.fire({
                              title: "Oops!",
                              text: "Must put quantity greater than 0 to each item to proceed!.",
                              icon: 'warning'
                         }).then((result)=>{
    ...
    function rateRoom(that,F_id){
            $.ajax({
                url:'ajax/rateRoom.php',
                type:'GET',
                data:{
                  rate:that,
                  room_id:F_id
               ...
               if (data > 0) {
                                  Swal.fire({
                                      title: "Thank you for rating our Room!",
                                      // text: "!!",
                                      icon: 'success'
                                 }).then((result)=>{
                                     window.location.reload();
    ...
    $("#registerForm").submit(function(e){
      ...
      Swal.fire({
        ...
        $("#otpcodeForm").submit(function(e){
      e.preventDefault();
      Swal.fire({
                    title: 'Submit?!',
                    text: "Are you sure? please double check the code!",
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonColor: '#3085d6',
                    cancelButtonColor: '#d33',

...
if (isset($otpp)) {
	...
	$check ="SELECT * FROM `user_otp` WHERE `u_id`='$otpp' AND `otp_code`='$theotpcode' AND `otp_status`=1 ORDER BY `otp_id` DESC Limit 1";
	$cquery = $con->query($check);
	$nrow = $cquery->num_rows;
	if ($nrow > 0) {
		$crow = $cquery->fetch_array();
		$otp_date = $crow['otp_date'];
		$plus5 = date("Y-m-d H:i:s", strtotime($otp_date.'+ 5 minutes'));
		$date1 = new DateTime($plus5);
		$date2 = new DateTime($todatetime);
		if ($date1 >= $date2) {
			$_SESSION['rrid'] = $otpp;
			echo "1";
		}else{
		...
		$todatetime = date("Y-m-d H:i:s");
	$check ="SELECT * FROM `user_otp` WHERE `u_id`='$otpp' AND `otp_code`='$theotpcode' AND `otp_status`=0 ORDER BY `otp_id` DESC Limit 1";
	$cquery = $con->query($check);
	$nrow = $cquery->num_rows;
	if ($nrow > 0) {
		$crow = $cquery->fetch_array();
...
// }
	...
	if (isset($orderCanceled)) {
		$today = date("Y-m-d");
		 $update = "UPDATE `orderdetails` SET `delivery_status`= 4,`delivery_date`='$today' WHERE `order_id`='$order_id'";
		$uquery = $con->query($update);

		$message = "The order was Canceled with order No.".$order_id;
		$ch = curl_init(); 
		 $parameters = array(
		          'apikey' => '2333c22cb66e930b71c0246618ca3776', 
		          'number' => '09063800777',
